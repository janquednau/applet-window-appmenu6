...
void SchemesModel::setCurrentOptionIsShown(bool isShown)
{
    ...
    Q_EMIT currentOptionIsShownChanged();
}
    ...
    switch (role) {
        case Qt::DisplayRole:
            ...
            if (index.row() == 1 && m_currentOptionIsShown) {
                return QStringLiteral("Current Window");
            }
        ...
        case Qt::UserRole + 4:
            if (index.row() == 0) {
                return QStringLiteral("_default_");
            }

            if (index.row() == 1 && m_currentOptionIsShown) {
                return QStringLiteral("_current_");
            }
...
QColor SchemesModel::backgroundOf(const int &index) const
{
    ...
    return QColor(QStringLiteral("transparent"));
}
...
void SchemesModel::initSchemes()
{
    ...
    QString currentSchemePath = SchemeColors::possibleSchemeFile(QStringLiteral("kdeglobals"));
    insertSchemeInList(currentSchemePath);

    QStringList standardPaths = AppletDecoration::standardPathsFor(QStringLiteral("color-schemes"));
...
{
    if (file.isEmpty() || file == QLatin1String("_default_")) {
        return 0;
    }

    if (file == QLatin1String("_current_")) {
        return 1;
    }

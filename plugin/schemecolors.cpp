...
QString SchemeColors::schemeFile() const
{
    return m_schemeFile;
}
...
{
    if (scheme.startsWith(QLatin1Char('/')) && scheme.endsWith(QStringLiteral("colors")) && QFileInfo(scheme).exists()) {
        return scheme;
    }
    ...
    if (scheme == QLatin1String("kdeglobals")) {
        QString settingsFile = QDir::homePath() + QStringLiteral("/.config/kdeglobals");
        ...
        if (QFileInfo(settingsFile).exists()) {
            ...
            tempScheme = generalGroup.readEntry("ColorScheme", QString());
        }
    }
    ...
    return AppletDecoration::standardPath(QStringLiteral("color-schemes/") + schemeNameSimplified + QStringLiteral(".colors"));
}
